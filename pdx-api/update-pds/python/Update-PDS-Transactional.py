...
def update_pds():
    """
    Update a row in the ds_referrals dataset for this PDS.
    """
    payload = {
        "status": "complete",
        "type": "signpost"
    ...
    url = f"{MYDEX_PDS_PATH}/api/pds/transaction/{MEMBER_UID}.json"
    headers = {
        'Authorization': f"Bearer {oauth_access_token}",
        'Content-Type': 'application/json'
    }

    response = requests.put(url, headers=headers, params=query_params, data=json.dumps(payload))
    response.raise_for_status()
